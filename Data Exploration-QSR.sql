...
--------------------------------------------------------------------------------------------------------------------------------------------
/* 14- find monthly spend and sort it by descending order. Output should contain columns- Year, Month, totalprice*/

Select Year(a.created_date) as year, date_format(a.created_date, "%m") as month, sum(a.pri) as monthly_spend
from (
select s.*, p.product_name, p.price as pri
from sales as s
join product as p
on s.product_id=p.product_id
order by s.created_date) as a
Group by 1,2
order by 1,2;
